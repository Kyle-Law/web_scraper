...
class Scraper
  ...
  def indeed_scraper
    ...
    page_start_num.each do |start|
      ...
      jobs_listings.each do |listing|
        jobs << if listing.css('span.date').text != 'Today'
                  { title: listing.css('a.jobtitle').text, company: listing.css('span.company').text, location: listing.css('span.location').text, summary: listing.css('div.summary').text.gsub("\n", '-'), day_posted: listing.css('span.date').text }
                else
  ...
  def remoteio_scraper
    ...
    jobs = []
    if total_pages != 0
      puts "found a total of #{parsed_page.css('small').text[1, 3]} jobs, with a total of #{total_pages} pages"
    end
